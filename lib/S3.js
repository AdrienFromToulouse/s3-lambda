      ...
      this.s3Instance.getObject({
        ...
        if (err) {
          ...
          try {
            ...
            if (this.verbose) {
              console.info(`GET OBJECT s3://${bucket}/${key}`) // eslint-disable-line
            }
          } catch (e) {
            fail(e)
          }
    ...
    return new Promise((success, fail) => {
      this.s3Instance.putObject({
        ...
        if (err) {
          ...
          if (this.verbose) {
            console.info(`PUT OBJECT s3://${bucket}/${key}`) // eslint-disable-line
          }
          success(res)
        }
      })
    ...
    return new Promise((success, fail) => {
      this.s3Instance.copyObject({
        ...
        if (err) {
          ...
          if (this.verbose) {
            console.info(`COPY OBJECT s3://${bucket}/${key} --> s3://${targetBucket}/${targetKey}`) // eslint-disable-line
          }
          success()
        }
      })
    ...
    return new Promise((success, fail) => {
      this.s3Instance.deleteObject({
        ...
        if (err) {
          ...
          success()
          if (this.verbose) {
            console.info(`DELETE OBJECT s3://${bucket}/${key}`) // eslint-disable-line
          }
        }
      })
    })
  ...
  listRecursive(bucket, prefix, endPrefix, marker, allKeys, done) {
    ...
    this.s3Instance.listObjects({
      ...
      if (err) {
        ...
        if (this.verbose) {
          console.info(`LIST OBJECTS s3://${bucket}/${marker === '' ? prefix : marker}`) // eslint-disable-line
        }
